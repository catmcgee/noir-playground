...
export default function Home() {
  ...
  const [code, setCode] = useState("use dep::std\n\nfn main() {\n}");
  const [proverInputs, setProverInputs] = useState<
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(server_url + "challenges/" + challenge);
  ...
  const testCode = async () => {
    ...
    const response = await fetch(`${server_url}execute_test`, {
      method: "POST",
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
  ...
  const checkCode = async () => {
    setIsRunning(true);
    setResult("");
    console.log("require dotenv", process.env.NEXT_PUBLIC_SERVER_URL); // remove this a
    ...
    const response = await fetch(`${server_url}execute_check`, {
      method: "POST",
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
    ...
    if (response.ok) {
      const proverInputsArray = Object.entries(result).map(
        ([key, value], id) => ({ id, key, value: String(value) })
      );
  ...
  return (
    <div
      className="h-screen w-full p-5 flex flex-col"
        ...
        <div className="w-6/12 ml-5 flex flex-col">
            ...
            <Tabs>
              ...
              <TabPanel>
                <ProverInput
                  ...
                  setProverInputs={setProverInputs}
                />
                {proverInputs.length === 0 && (
                  <p className="text-sm text-gray-500 mt-2">
                    Prover Inputs are the inputs supplied in Prover.toml, which
                    the prover uses to send its witness values(both private and
                    public).
                  </p>
                )}
              </TabPanel>
          ...
          <div className="flex mt-4">
            <div className="flex mt-4 justify-between">
              ...
              <button
                onClick={testCode}
                className="w-full py-2 px-4 bg-blue-500 text-white rounded mx-2"
              ...
              <button
                onClick={submitCode}
                className="w-full py-2 px-4 bg-green-500 text-white rounded ml-2"
                style={{ backgroundColor: "#4db324" }}
              >
