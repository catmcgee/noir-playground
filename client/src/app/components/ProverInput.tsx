...
type ProverInputProps = {
  ...
  setProverInputs: (proverInputs: Array<{ id: number, key: string, value: string }>) => void;
};

const ProverInput: React.FC<ProverInputProps> = ({ proverInputs, setProverInputs }) => {
...
const handleAddProverInput = () => {
  ...
  return (
    <div className="mt-2">
      ...
      <div className="flex gap-2 mb-2"> {/* Add key-value pair section */}
        <button 
          onClick={handleAddProverInput} 
          className="w-2/12 py-2 px-4 bg-blue-500 text-white rounded hover:bg-blue-600"    style={{ backgroundColor: '#9f3fff' }}> 
            Add
        </button>
      </div>
        ...
        <li key={id} className="mb-2">
          <div className="flex justify-between">
            <input 
              type="text"
              value={key} 
              onChange={e => setProverInputs(proverInputs.map(
                input => input.id === id ? { ...input, key: e.target.value } : input
              ))}
            ...
            <input 
              ...
              value={value} 
              onChange={e => {
                setProverInputs(proverInputs.map(input =>
            ...
            <button onClick={() => handleDeleteProverInput(id)} className="w-2/12 text-center">🗑️</button>
          </div>
        </li>
      ))}
